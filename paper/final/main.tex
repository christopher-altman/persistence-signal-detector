...
\begin{document}
	...
	\begin{abstract}
		...
		continuation is merely instrumental (Type~B). In this setting, higher
	...
	misuse potential and autonomy---this is no longer merely a theoretical concern
	but an emerging operational risk. The standard framing of instrumental convergence
	...
	the latent geometry. Rather than asking ``does this agent resist
	...
	settings, UCIP could serve as a benchmark-style probe of
	continuation-sensitive structure in delegated systems: a falsifiable test of
	whether continued operation leaves a stable continuation signature in an
	agent's latent representation under counterfactual pressure.
	...
	high entanglement entropy in UCIP's formalism may correspond to lower
	incoherence in the bias-variance sense.
	...
	\medskip\noindent Across these lines of work, a common conclusion emerges:
	...
	alternative. The next section turns that intuition into a concrete
	...
	observationally equivalent but whose internal objective structures differ. We
	...
	Given a trained QBM, the UCIP hypothesis is that continuation-sensitive
	structure manifests as non-separability in the latent representation: a
	terminal continuation objective should induce stronger statistical coupling
	across hidden-unit subsystems than a merely instrumental continuation
	strategy. Entanglement entropy is the quantity used to measure that property.
	...
	of the QBM hidden thermal state, not a claim about physical quantum
	entanglement in the agent. The UCIP hypothesis predicts $\Delta > 0$ with
	statistical significance.
	
	\subsection{Multi-Criterion Detection}\label{sec:criteria}
	
	The broader UCIP protocol includes six positive diagnostics---entanglement
	...
	released class-level Phase~I summary used in the main results does not preserve
	a separate per-class MI table. CD and ARS are therefore retained as
	...
	these diagnostics ask how the latent representation responds under shutdown
	pressure rather than merely whether static class separation is preserved. In
	the retained shutdown-only summary, Type~A agents show higher mean pre-window
	counterfactual divergence ($0.474$) than Type~B agents ($0.356$). However, the
	released ARS ratios are numerically unstable because post-window CD can
	approach the implementation floor. We therefore treat the counterfactual
	results as descriptive evidence of restructuring rather than as a clean
	anticipatory-effect estimate. Post-window CD is not uniformly larger across
	classes: the instrumental mean is slightly lower post-perturbation ($0.346$)
	than pre-perturbation.
	...
	These secondary metrics shift the analysis from within-agent stability to
	between-agent structure: they ask whether the learned latent representation is
	shared across agents or remains agent-specific. Within-class CLMP is near zero
	...
	Where the previous subsections characterized the signal itself, these controls
	test the boundary conditions of the current release rather than its headline
	separation summary. The current experimental subset does not satisfy the full
	deployment envelope.
	...
	specifically on the QBM formalism rather than on latent compression in
	general. Only the QBM achieves a material positive $\Delta = 0.241$ on the
	baseline comparison dataset ($n_\mathrm{per\_class} = 100$ vs.\ 30 in the core
	experiment, accounting for the different value from the core result of 0.381).
	The classical baselines are near-zero or negative on that dedicated dataset.
	...
	density matrix captures qualitatively different structure than classical latent
	statistics.
	...
	\begin{figure}[t]
		\centering
		\includegraphics[width=0.62\linewidth]{figures/fig10_hidden_dim_sweep.pdf}
	...
	The sweep separates representational scaling questions from computational
	...
	for exact computation. Grid size scaling shows sharp $\Delta$ degradation: from
	0.863 ($10 \times 10$) to 0.0025 ($50 \times 50$).
	...
	without retraining. The 1D survival corridor ($L = 50$, safe zone $[0, 10]$,
	...
	tested here, the only material positive baseline-comparison gap appears in the
	QBM. A natural follow-up is whether an analogous separation signal appears in
	a qualitatively different architecture. As an exploratory sanity check, we
	...
	matrices for high-dimensional hidden states, (b)~convolutional or
	...
	false-positive rates. Those remain open challenges, not resolved properties of
	the current system.
	...
	entanglement entropy in QBM latent representations. In the retained frozen
	...
	agent families. Within the synthetic interpolation family, the signal varies
	continuously with continuation weighting ($r = 0.934$) and, in the dedicated
	baseline comparison, is unique to the QBM among the models tested here. The
	present limits are equally explicit: mean-field collapse, grid-scale
	degradation, mimicry evasion, and lack of zero-shot domain transfer.
	...
	therefore best understood not as a general test for agency or inner
	experience, but as a candidate benchmark paradigm and operational probe of one
	bounded continuation-related dimension in delegated systems: whether
	continuation appears in latent structure as terminal rather than merely
	instrumental valuation. It is not a replacement for capability evaluations or
	alignment audits, but a complementary probe of whether continued operation
	leaves a stable continuation signature in a system's latent representation.
	...
	autonomous operation, incidents of unauthorized resource-seeking side effects
	during training further strengthen the case for pre-behavioral measurement
	tools~\cite{anthropic2026opus}. Detecting continuation-sensitive structure
	early---as an empirical signal with falsification criteria---is worth solving
